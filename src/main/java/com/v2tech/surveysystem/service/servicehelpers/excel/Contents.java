...
public class Contents {
	...
	protected static com.itextpdf.text.Font catFont = new com.itextpdf.text.Font(
			com.itextpdf.text.Font.FontFamily.TIMES_ROMAN, 18,
			com.itextpdf.text.Font.BOLD);
	protected static com.itextpdf.text.Font redFont = new com.itextpdf.text.Font(
			com.itextpdf.text.Font.FontFamily.TIMES_ROMAN, 12,
			com.itextpdf.text.Font.NORMAL, BaseColor.RED);
	protected static com.itextpdf.text.Font subFont = new com.itextpdf.text.Font(
			com.itextpdf.text.Font.FontFamily.TIMES_ROMAN, 16,
			com.itextpdf.text.Font.BOLD);
	protected static com.itextpdf.text.Font smallBold = new com.itextpdf.text.Font(
			com.itextpdf.text.Font.FontFamily.TIMES_ROMAN, 9,
			com.itextpdf.text.Font.NORMAL, BaseColor.BLACK);
	protected static com.itextpdf.text.Font small = new com.itextpdf.text.Font(
			com.itextpdf.text.Font.FontFamily.TIMES_ROMAN, 8,
			com.itextpdf.text.Font.NORMAL, BaseColor.BLACK);

	protected static void createCompanyInformationForExcel(
			XSSFWorkbook xssfWorkbook, XSSFSheet sheet, String templateName) {
		// Company info
		...
		createStringCell(xssfWorkbook, row4, (short) 0, CellStyle.ALIGN_LEFT,
				CellStyle.VERTICAL_BOTTOM, true, "Name : ");
		createStringCell(xssfWorkbook, row4, (short) 1, CellStyle.ALIGN_LEFT,
				CellStyle.VERTICAL_BOTTOM, true, "Puneet");

		Row row5 = sheet.createRow(5);
		createStringCell(xssfWorkbook, row5, (short) 0, CellStyle.ALIGN_LEFT,
				CellStyle.VERTICAL_BOTTOM, true, "Date : ");
		createDateCell(xssfWorkbook, row5, (short) 1, CellStyle.ALIGN_LEFT,
				CellStyle.VERTICAL_BOTTOM, true, new Date());

	}

	public static void createDataForTemplate1(XSSFWorkbook xssfWorkbook,
			XSSFSheet sheet, Map<String, List<QuestionAnswer>> map) {
		sheet.createRow(6);
		...
		createQuestionAnswerDataForExcelTemplate1(xssfWorkbook, sheet,
				map.get("Business Perspective".toLowerCase()));
		createQuestionAnswerDataForExcelTemplate1(xssfWorkbook, sheet,
				map.get("Problem Solving".toLowerCase()));
		createQuestionAnswerDataForExcelTemplate1(xssfWorkbook, sheet,
				map.get("Communication Skills".toLowerCase()));
		createQuestionAnswerDataForExcelTemplate1(xssfWorkbook, sheet,
				map.get("Influencing Skills".toLowerCase()));
		createQuestionAnswerDataForExcelTemplate1(xssfWorkbook, sheet,
				map.get("Self-Awareness".toLowerCase()));
		createQuestionAnswerDataForExcelTemplate1(xssfWorkbook, sheet,
				map.get("defaultGroup".toLowerCase()));
		// sheet.autoSizeColumn(1, true);
	}

	public static Cell createStringCell(XSSFWorkbook workbook, Row row,
			short column, short halign, short valign, boolean border,
			String cellValue) {
	...
	public static Cell createDateCell(XSSFWorkbook workbook, Row row,
			short column, short halign, short valign, boolean border,
			Date cellValue) {
	...
	protected static Document createPdfBasicHeader(Document document,
			String templateName) throws DocumentException,
			FileNotFoundException {

		document.open();
		document.addTitle("Assistment Report");
		document.addSubject("Pre-Workshop Competency Assessment (Manager)");
		...
		preface.add(new Paragraph(
				"Pre-Workshop Competency Assessment (Manager)",
		...
		table.setTotalWidth(new float[] { 100, 200 });
		table.setLockedWidth(true);
		table.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.addCell("Your Name");
		table.addCell("puneet");
		table.addCell("Email ");
		table.addCell("puneet.sharma@gmail.com");
		document.add(table);
	...
	protected static PdfPTable createPdfHeaderTemplate1(
			Map<String, List<QuestionAnswer>> map) throws DocumentException {
		PdfPTable table = new PdfPTable(10);
		table.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.setTotalWidth(new float[] { 50, 200, 100, 100, 100, 120, 100,
				100, 100, 100 });
		PdfPCell cell = new PdfPCell(
				new Phrase(
						"Please put an ‘x’ in the box on the right of each statement to reflect your current competency level.",
						smallBold));
		cell.setColspan(5);
		cell.setRowspan(2);
		table.addCell(cell);
		...
		table.addCell(c9);
		table.addCell(c10);
		createQuestionAnswerDataForPdfTemplate1(table,
				map.get("Business Perspective".toLowerCase()));
		createQuestionAnswerDataForPdfTemplate1(table,
				map.get("Problem Solving".toLowerCase()));
		createQuestionAnswerDataForPdfTemplate1(table,
	...
	public static void createQuestionAnswerDataForPdfTemplate1(PdfPTable table,
		...
		while (iterator.hasNext()) {
			...
			if (isVertical == true) {
				PdfPCell qroupCell = new PdfPCell(new Phrase(questionQroup,
						small));
			...
			if (answer != null) {
				if (answer.trim().equalsIgnoreCase("Very Poor")) {
					...
					table.addCell(x);
					table.addCell("");
				} else if (answer.trim().equalsIgnoreCase("Excelent")) {
					table.addCell("");
					table.addCell("");
					table.addCell("");
					table.addCell("");
	...
	public static void createQuestionAnswerDataForExcelTemplate1(
			XSSFWorkbook xssfWorkbook, XSSFSheet sheet,
			List<QuestionAnswer> questionAnswers) {
		if (questionAnswers == null) {
			return;
		}
		...
		while (iterator.hasNext()) {
			...
			if (isVertical == true) {
				...
				sheet.addMergedRegion(new CellRangeAddress(rowNumber, rowNumber
						+ questionAnswers.size() - 1, 0, 0));
				bvertical.setCellStyle(myStyle);
				isVertical = false;
			} else {
				/*
				 * XSSFCellStyle myStyle = xssfWorkbook.createCellStyle();
				 * myStyle.setAlignment(CellStyle.ALIGN_CENTER); myStyle
				 * .setVerticalAlignment(CellStyle.ALIGN_CENTER );
				 * myStyle.setRotation((short) 90); Cell bvertical =
				 * row.createCell(0); bvertical.setCellStyle(myStyle);
			...
			sheet.addMergedRegion(new CellRangeAddress(rowNumber, rowNumber, 1,
					5));
			rowNumber++;
		}
	}

	public static void createDataForExcelTemplate3(XSSFWorkbook xssfWorkbook,
			XSSFSheet sheet, Map<String, List<DoubleQuestionAnswer>> map, LeadershipSurveyResult leadershipSurveyResult) {
		sheet.createRow(6);
		Row row7 = sheet.createRow(7);
		sheet.createRow(8);
		sheet.createRow(9);
		Cell r7c1 = row7.createCell(0);
		r7c1.setCellValue("Put an ‘x’ in the clear box to the left of the statement in each pair that you think most accurately describes you.\nÏSometimes you may think neither or both apply.\nNevertheless, choose the statement which most closely describes you.\nAfter completing all 30, please add up the number of Xs in each column.");
		...
		r10c3.setCellValue("E");
		Cell r10c4 = row10.createCell(4);
		r10c4.setCellValue("C");
		...
		while (iterator.hasNext()) {
			...
			createQuestionAnswerDataForExcelTemplate2(xssfWorkbook, sheet,
					map.get(key));
		}
		int rowNumber = sheet.getPhysicalNumberOfRows();
		Row r11 = sheet.createRow(rowNumber);
		Row r12 = sheet.createRow(rowNumber + 1);
		Cell r11c0 = r12.createCell(0);
		r11c0.setCellValue("Total Score");
		Row r13 = sheet.createRow(rowNumber + 1);
		...
		r13c3.setCellValue("E");
		Cell r13c4 = r13.createCell(4);
		r13c4.setCellValue("C");
		Row r14 = sheet.createRow(rowNumber + 1);
		// Values
		...
		r14c3.setCellValue(leadershipSurveyResult.getNumE());
		Cell r14c4 = r14.createCell(4);
		r14c4.setCellValue(leadershipSurveyResult.getNumC());
	}

	public static void createQuestionAnswerDataForExcelTemplate2(
			XSSFWorkbook xssfWorkbook, XSSFSheet sheet,
			List<DoubleQuestionAnswer> questionAnswers) {
		if (questionAnswers == null) {
			return;
		}
	...
	protected static PdfPTable createPdfDataTemplate3(
			Map<String, List<DoubleQuestionAnswer>> map)
			throws DocumentException {
		PdfPTable table = new PdfPTable(6);
		table.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.setTotalWidth(new float[] { 50, 50, 50, 50, 50, 800 });
		table.addCell("");
	...
	protected static PdfPTable createPdfResultForTemplate3(
			LeadershipSurveyResult leadershipSurveyResult)
			throws DocumentException {
		PdfPTable table = new PdfPTable(6);
		table.setHorizontalAlignment(Element.ALIGN_LEFT);
		table.setTotalWidth(new float[] { 50, 50, 50, 50, 50 });
		table.addCell("Total Scores");
		table.addCell("");
		table.addCell("");
		table.addCell("");
		table.addCell("");
	...
	public static void createQuestionAnswerDataForPdfTemplate3(PdfPTable table,
			List<DoubleQuestionAnswer> questionAnswers) {
		if (questionAnswers == null) {
			return;
		}
		...
		while (iterator.hasNext()) {
			...
			QuestionAnswer questionAnswer1 = doubleQuestionAnswer
					.getQuestionAnswer1();
			...
			table.addCell(questionCell);
			QuestionAnswer questionAnswer2 = doubleQuestionAnswer
					.getQuestionAnswer2();
